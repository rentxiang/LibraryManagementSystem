...
-- By bookName
GO
ALTER PROCEDURE searchBook @bookName VARCHAR(30), @personID VARCHAR(10), @message VARCHAR(100) OUTPUT
AS
BEGIN
	-- DECLARE @personID VARCHAR(10)
...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM Book WHERE BookID = @bookID AND Number_Of_Copies > 1)
		BEGIN
			...
			-- IF EXISTS (SELECT 1 FROM Reader WHERE ReaderID = @readerID AND No_of_Books_Issued > 0)
...
-- Stored procedure for print functionality
GO
ALTER PROCEDURE GetAvailablePrinters AS
BEGIN
	SELECT * FROM Material WHERE MaterialType = 'Printer' AND MaterialStatus = 'Available';
END

GO
ALTER PROCEDURE requestPrint 
	@personID VARCHAR(10), 
